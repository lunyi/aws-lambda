...
WORKDIR /src
COPY ["lambda-net6-container-image.csproj", "lambda-net6-container-image/"]
RUN dotnet restore "lambda-net6-container-image/lambda-net6-container-image.csproj"

WORKDIR "/src/lambda-net6-container-image"
COPY . .
RUN dotnet build "lambda-net6-container-image.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "lambda-net6-container-image.csproj" \
...
Execute unit tests
```
cd "lambda-net6-container-image/test/lambda-net6-container-image.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "lambda-net6-container-image/src/lambda-net6-container-image"
