...
WORKDIR /src
COPY ["serverless-net6-container-image.csproj", "serverless-net6-container-image/"]
RUN dotnet restore "serverless-net6-container-image/serverless-net6-container-image.csproj"

WORKDIR "/src/serverless-net6-container-image"
COPY . .
RUN dotnet build "serverless-net6-container-image.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "serverless-net6-container-image.csproj" \
...
Execute unit tests
```
    cd "serverless-net6-container-image/test/serverless-net6-container-image.Tests"
    dotnet test
```

Deploy application
```
    cd "serverless-net6-container-image/src/serverless-net6-container-image"
